...
static pthread_cond_t incoming_street = PTHREAD_COND_INITIALIZER;
static pthread_cond_t outgoing_street = PTHREAD_COND_INITIALIZER; 
static pthread_cond_t street = PTHREAD_COND_INITIALIZER;
...
void incoming_enter() {
	...
	while (USAGE_LIMIT < cars_since_repair)
		pthread_cond_wait(&repair,&mtx);

	while (outgoing_onstreet >= 1)
...
void outgoing_enter() {
	...
	while (incoming_onstreet >= 1)
		pthread_cond_wait(&incoming_street,&mtx);
